...
-- -----------------------------------------------------
-- Placeholder table for view `mdp`.`linkablecompanyworks`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mdp`.`linkablecompanyworks` (`name` INT, `url` INT, `work` INT, `company` INT, `role` INT);
...
-- -----------------------------------------------------
-- View `mdp`.`linkablecompanyworks`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mdp`.`linkablecompanyworks`;
USE `mdp`;
CREATE  OR REPLACE VIEW `linkablecompanyworks` AS
SELECT w.shortName AS name, imgs.url AS url,wpp.work AS work, wpp.company AS company, roles.name AS role 
FROM workcompanyparticipations AS wpp 
INNER JOIN works AS w ON wpp.work = w.id 
INNER JOIN (SELECT work, url FROM workimages WHERE workimages.type = 1) AS imgs ON w.id = imgs.work 
INNER JOIN roles ON roles.id = wpp.role;
