...
INNER JOIN (SELECT work, url FROM workimages WHERE workimages.type = 0) AS imgs ON p.id = imgs.work 
INNER JOIN roles ON roles.id = wpp.role;
...
INNER JOIN (SELECT company, url FROM copmanyimages WHERE companyimages.type = 0) AS imgs ON c.id = imgs.company;
